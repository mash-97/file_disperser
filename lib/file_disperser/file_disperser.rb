...
module FileDisperser
  ...
  class FileDisperser
    ...
    def self.disperse_files(src_files_path_hash, dest_directories)
      ...
      src_files_path_hash.each do
        ...
        number.times do
          ...
          dest_file_name = File.uniqFin(File.basename(source_file_path), dest_directory)
    ...
    def run(dispersing_method, fgv=10)
      ...
      src_files_path_hash = @source_files.relate_with_as_hash_value(number)
      ...
      if fgv==nil then
        $fdspsr_log.debug("Calling FolderGenerator for #{@dest_dir} :: with value: #{fgv}")
        fg = FileDisperser::FolderGenerator.new(@dest_dir, limits: {dispersion_range: (5..fgv), depth_limit: fgv})
        dest_directories = fg.gen_folders()
      else
